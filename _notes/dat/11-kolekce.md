# DAT 11 — Kolekce: pole, zásobník, fronta, slovník

> **Cíl:** umět o tématu mluvit 10–15 min s komisí, zvládnout 30 min praktickou úlohu (bistro — kombinace Dict + Queue + Stack).
> **Předmět:** DAT / okruh **PRG** (programování)
> **Souvisí s:** **SWI 9** (OOP — kolekce jsou generic třídy s `<T>`), DAT 8 (pole — statická základní struktura), DAT 9 (spojové struktury — alternativa pole), DAT 10 (lambda + LINQ pro filtraci kolekcí), SWI 2 (algoritmická složitost — `O(1)` vs `O(n)`)
